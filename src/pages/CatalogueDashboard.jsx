...
import { Package, AlertTriangle, ImageOff, DollarSign, FolderTree, RefreshCw, Clock, Plus, Upload, Download, FileSpreadsheet } from "lucide-react";
...
function StatCard({ icon: Icon, label, value, color = "gray", link }) {
  const colorMap = {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={Package} label="Active Items" value={s.total_active || 0} color="green" />
        <StatCard icon={Package} label="Inactive/Discontinued" value={s.inactive_discontinued || 0} color="gray" />
        <StatCard icon={DollarSign} label="Missing Pricing" value={s.missing_pricing || 0} color="red" />
        <StatCard icon={FileSpreadsheet} label="Missing BT Fields" value={s.missing_bt_fields || 0} color="amber" />
        <StatCard icon={ImageOff} label="Missing Images" value={s.missing_images || 0} color="amber" />
        <StatCard icon={FolderTree} label="Incomplete Options" value={s.incomplete_option_groups || 0} color="purple" />
        <StatCard icon={AlertTriangle} label="Not Assigned to Any Project" value={s.not_assigned_to_any_project || 0} color="blue" />
        <StatCard icon={Clock} label="Needing Review" value={s.needing_review?.length || 0} color="amber" />
      </div>
