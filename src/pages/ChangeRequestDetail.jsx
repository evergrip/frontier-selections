...
export default function ChangeRequestDetail() {
  ...
  async function updateStatus(status) {
    ...
    try {
      ...
      toast({ title: `Change request ${status.toLowerCase()}`, description: status === "Approved" ? "The selection has been updated and procurement adjusted." : undefined });
  ...
  async function toggleLock() {
    ...
    try {
      ...
      toast({ title: next ? "Change request locked" : "Change request unlocked" });
      window.dispatchEvent(new Event("frontier:data-updated"));
      load();
    } catch (e) {
      toast({ title: "Failed to toggle lock", description: e.message || "Unknown error", variant: "destructive" });
    }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <Button variant="outline" size="sm" onClick={toggleLock} disabled={lockLoading} className="gap-2">{cr.is_locked ? <Lock size={14} /> : <Unlock size={14} />} {lockLoading ? "..." : cr.is_locked ? "Locked" : "Lock"}</Button>
      </div>
      ...
      {!resolved && !cr.is_locked && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" className="gap-1 bg-emerald-600 hover:bg-emerald-700" onClick={() => updateStatus("Approved")} disabled={statusLoading}><Check size={14} /> {statusLoading ? "Processing..." : "Approve"}</Button>
            <Button size="sm" variant="outline" className="gap-1" onClick={() => updateStatus("More Information Needed")} disabled={statusLoading}><MessageCircle size={14} /> Request Info</Button>
            <Button size="sm" variant="outline" className="gap-1 text-red-600" onClick={() => updateStatus("Rejected")} disabled={statusLoading}><X size={14} /> Reject</Button>
            <Button size="sm" variant="outline" onClick={() => updateStatus("Under Review")} disabled={statusLoading}>Mark Under Review</Button>
          </div>
        </div>
      )}
